...
MIN_THROW_VX = -0.2  # Relaxed from -0.3
MIN_SPEED = 0.3      # Relaxed from 0.5
MIN_HEIGHT = 0.6     # Relaxed from 0.8

# Gripper timing
GRIPPER_TRIGGER_DISTANCE = 0.25  # Start from more conservative distance
GRIPPER_CATCH_DISTANCE = 0.12

# Constants
GRAVITY = np.array([0.0, 0.0, -9.81])
MAX_INTERCEPT_TIME = 5.0  # Increased to handle slower throws
    ...
    for offset in [-0.5, -0.6, -0.7, -0.8, -0.9, -1.0]:
        ...
        if block_pos[0] > catch_x:
            ...
            if vx < -0.1:  # Moving towards catcher
                ...
                if 0.2 < dt < 4.0:  # Reasonable time window
        ...
        if dt is not None and pred_pos is not None:
            ...
            if joints is not None:
                ...
                if not gripper_closing and not gripper_closed:
                    ...
                    elif dt < 0.25:  # Also trigger on time
                        should_close = True
